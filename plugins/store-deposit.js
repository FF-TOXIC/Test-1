...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if (conn.deposit[m.sender])
    throw "*[ SELESAIKAN TRANSAKSI MU SEBELUM DEPOSIT KEMBALI ]*";
  ...
  try {
    ...
    let caption = `*[ DEPOSIT SALDO BY AKIRAA-BOT ]*
...
handler.before = async (m, { conn }) => {
  ...
  if (conn.deposit[m.sender].id) {
    if (cek.msg === "SUCCESS") {
      global.db.data.users[m.sender].saldo += conn.deposit[m.sender].nominal;
...
*Terimakasih telah menggunakan AkiraaBot, Saldo Sudah masuk ke akun anda, untuk mengecek silahkan ketik .ceksaldo*`;
